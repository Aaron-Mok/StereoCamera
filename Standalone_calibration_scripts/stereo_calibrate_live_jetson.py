...
TODO: This is trying to use findChessboardCOrnerSB, which does not seems to work.
Can switch back to findChessboardCorners if needed, but SB is supposed to be more robust and accurate. 
"""
import sys
import os
...
from ISP.binning import extract_green, bin_2x2
...
# extract_green → (H//2, W//2), then bin_2x2 → (H//4, W//4) = (540, 960)
IMG_W, IMG_H = W // 4, H // 4
...
def raw_to_gray8(raw_u16: np.ndarray) -> np.ndarray:
    """Extract green channel, 2×2 bin to 960×540, convert to uint8."""
    return bit16_to_bit8(bin_2x2(extract_green(raw_u16)))


def find_corners(gray, pattern_size):
    """Detect directly at 960×540. SB includes subpixel — no further scaling needed."""
    sb_flags = cv2.CALIB_CB_NORMALIZE_IMAGE
    return cv2.findChessboardCornersSB(gray, pattern_size, sb_flags)
...
def main():
    ...
    print(f"[i] Press 's' to run calibration (need at least {MIN_PAIRS} pairs).")
    ...
    while True:
        ...
        foundL, cornersL = find_corners(grayL, pattern_size)
        foundR, cornersR = find_corners(grayR, pattern_size)

        # 960×540 is directly displayable — no resize needed
        showL = grayL.copy()
        showR = grayR.copy()
        ...
        if key == ord('q'):
            ...
            while True:
                ...
                gpu_gL2 = cv2.cuda_GpuMat(); gpu_gL2.upload(gL2)
                gpu_gR2 = cv2.cuda_GpuMat(); gpu_gR2.upload(gR2)
                gpu_rectL = cv2.cuda.remap(gpu_gL2, gpu_map1x, gpu_map1y, cv2.INTER_LINEAR)
                gpu_rectR = cv2.cuda.remap(gpu_gR2, gpu_map2x, gpu_map2y, cv2.INTER_LINEAR)

                # Resize to 960×540 on GPU, cvtColor, download for hconcat
                # 960×540 frames — cvtColor on GPU, hconcat on CPU → 1920×540
                bgrL = cv2.cuda.cvtColor(gpu_rectL, cv2.COLOR_GRAY2BGR).download()
                bgrR = cv2.cuda.cvtColor(gpu_rectR, cv2.COLOR_GRAY2BGR).download()
                both = cv2.hconcat([bgrL, bgrR])  # 1920×540 — directly displayable
